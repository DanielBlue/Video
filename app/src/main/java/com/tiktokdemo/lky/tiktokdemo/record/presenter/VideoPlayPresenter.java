...
//                    }
    ...
    public void combineVideo(final float originalVolume, final float backgroundVolume,String path) {
        mView.showLoadingView(true,0);
        mView.combineVideoStart();
        mBGMPath=path;
        File file=new File(mBGMPath);
        Log.d("jiejie", "combineVideo: "+mBGMPath);
        if(file.exists()){
            Log.d("jiejie", "exits: ");
        }else{
            Log.d("jiejie", "no-exits: ");
        }
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for(int i=0;i<strPath.size();i++){
                    ...
                    Log.d("jiejie", "run: "+strPath.get(i));
                ...
                heyhouVideo.combine(videoInfos, mBGMPath==null?"":mBGMPath,originalVolume,backgroundVolume, Constant.RECORD_VIDEO_PATH_TEMP + File.separator + "shibo_video" + ".mp4", new VideoListener() {
                    @Override
